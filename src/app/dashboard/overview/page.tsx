...
export default function AllGuestsOverviewPage() {
  ...
  const filtered = guests.filter(g => {
    ...
    return g.status !== 'ARCHIVED'; // "all" excludes archived
  });
    ...
    <div className="space-y-6 fade-in">
      ...
      <div className="flex gap-2 flex-wrap">
        {[
          ...
          { key: 'archived', label: 'Archived', count: guests.filter(g => g.status === 'ARCHIVED').length },
        ].map(tab => (
            ...
            return (
              <Link key={guest.id} href={`/dashboard/guests/${guest.id}`}
                className={`card hover:shadow-md transition-shadow cursor-pointer
                  ...
                  ${guest.status === 'ARCHIVED' ? 'opacity-60' : ''}`}>
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    {guest.assignedVolunteer && (
                      <p className="text-xs text-church-400 mt-0.5">→ {guest.assignedVolunteer.name}</p>
                    )}
                  </div>
